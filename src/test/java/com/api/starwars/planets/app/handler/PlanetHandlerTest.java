...
class PlanetHandlerTest extends AbstractIntegrationTest {
    ...
    class GetAll {
        ...
        void find_in_database_successfully() throws Exception {
            ...
            PlanetView planet = PlanetView.fromDomain(mongoPlanet.toDomain());
            mockMvc.perform(MockMvcRequestBuilders.get(Constants.PLANETS_ENDPOINT).header("Authorization", TOKEN))
                    .andExpect(status().isOk())
                    .andExpect(jsonPath("$.page").value(0))
                    .andExpect(jsonPath("$.size").value(15))
    ...
    class UpdateById  {
        ...
        void update_successful() throws Exception {
            MongoPlanet mongoPlanet = saveMongoPlanet(DomainUtils.getRandomMongoPlanet());
            PlanetView updatePlanet = DomainUtils.getRandomPlanetJson();
            updatePlanet.setId(mongoPlanet.getId().toString());

            mockMvc.perform(put(ENDPOINT)
                            .header("Authorization", TOKEN)
                            .contentType(MediaType.APPLICATION_JSON.getMediaType())
                            .content(mapper.writeValueAsString(PlanetView.fromDomain(updatePlanet.toDomain()))))
                    .andExpect(status().isOk())
                    .andExpect(jsonPath("$.id").value(mongoPlanet.getId().toString()))
        ...
        void throw_not_found() throws Exception {
            ...
            planetView.setId(UUID.randomUUID().toString());

            mockMvc.perform(put(ENDPOINT)
                            .header("Authorization", TOKEN)
                            .contentType(MediaType.APPLICATION_JSON.getMediaType())
                            .content(mapper.writeValueAsString(planetView)))
                    .andExpect(status().isNotFound())
                    .andExpect(jsonPath("$.message").value(format("Nenhum planeta com id {0} foi encontrado.", planetView.getId())));
        }

        @Test
        @DisplayName("Deve estourar 400 quando o planeta tiver campos invalidos.")
        void throw_bad_request() throws Exception {
            PlanetView planetView = DomainUtils.getInvalidPlanetJson();
            planetView.setId(UUID.randomUUID().toString());
    ...
    class Save  {
        ...
        void save_successful() throws Exception {
            ...
            mockMvc.perform(MockMvcRequestBuilders.post(Constants.PLANETS_ENDPOINT)
                            .header("Authorization", TOKEN)
                            .contentType(MediaType.APPLICATION_JSON.getMediaType())
                            .content(mapper.writeValueAsString(PlanetView.fromDomain(planetView.toDomain()))))
                    .andExpect(status().isCreated())
                    .andExpect(jsonPath("$.id", notNullValue()))
                    .andExpect(jsonPath("$.name").value(planetView.getName()))
                    .andExpect(jsonPath("$.climate", hasSize(planetView.getClimate().length)))
                    .andExpect(jsonPath("$.cacheInDays").value(0L));
        }
